...
class ResizableFramelessWindow(QMainWindow):
    ...
    def setup_resize_cursors(self, pos):
        ...
        elif pos.x() < self.border_width and pos.y() > rect.height() - self.corner_width:
            # 左下角
            self.setCursor(QCursor(Qt.CursorShape.SizeBDiagCursor))
            self.resize_direction = "bottom-left"
        elif pos.x() > rect.width() - self.border_width and pos.y() > rect.height() - self.corner_width:
            # 右下角
            self.setCursor(QCursor(Qt.CursorShape.SizeFDiagCursor))
    ...
    def mouseMoveEvent(self, event):
        if not self.is_maximized_state:
            ...
            new_x = event.globalPosition().toPoint().x() - int(new_size.width() * mouse_x_ratio)
